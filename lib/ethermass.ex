defmodule Ethermass do
  ...
  def get_explorer_link(address, part \\ "address") do
    case Application.get_env(:ethereumex, :network) do
      ...
      "goerli" -> "https://goerli.etherscan.io/#{part}/#{address}"
      _ -> "-"
    end
  end
  ...
  def private_key_to_address(private_key) do
    try do
      trimmed_priv_key =
        if String.starts_with?(private_key, "0x"),
          do: String.trim_leading(private_key, "0x"),
          else: private_key
