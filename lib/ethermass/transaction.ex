defmodule Ethermass.Transaction do
  ...
  def get_status_summary(transaction_batch_id) do
    ...
    %{
      failed_with_tx:
        result
        |> Enum.filter(fn x -> x.status == "failed" && not is_nil(x.hash) end)
        |> Enum.count(),
      failed_without_tx:
        result |> Enum.filter(fn x -> x.status == "failed" && is_nil(x.hash) end) |> Enum.count(),
      success: result |> Enum.filter(fn x -> x.status == "success" end) |> Enum.count(),
      wait_confirmation:
        result |> Enum.filter(fn x -> x.status == "wait_confirmation" end) |> Enum.count(),
  ...
  def run_send_eth_plan(%TransactionPlan{} = plan) do
    case update_transaction_plan(plan, %{"status" => "in_progress"}) do
      {:ok, plan} ->
        ...
        data = %{
          to: plan.to,
          gas_limit: plan.gas_limit |> to_hex,
          gas_price: (plan.gas_price * 1_000_000_000) |> to_hex,
          from: plan.from,
          value: value
        }
        ...
        case ETH.send_transaction(data, priv_key) do
          {:ok, hash} ->
            update_transaction_plan(plan, %{"status" => "wait_confirmation", "hash" => hash})
      ...
      {:error, _} ->
        {:error, "Cannot change status to in_progress"}
    end
  end
  ...
  def fund_address(from, to, value) do
    ...
    data = %{
      to: to,
      gas_limit: 100_000 |> to_hex,
      gas_price: (3 * 1_000_000_000) |> to_hex,
      from: from,
      value: value
    }
  ...
  def run_nft_whitelist_plan(%TransactionPlan{} = plan) do
    case ETH.Query.gas_price() do
      {:ok, wei} ->
        ...
        total_gas = if old_total_gas < 88, do: 88, else: old_total_gas

        if total_gas > plan.gas_price do
          update_transaction_plan(plan, %{
            "status" => "postponed",
            "remark" =>
              "Base gas price #{base_price_gwei} and priority fee is higher than limit #{plan.gas_price}"
          })
        else
          priv_key = Ethermass.Wallet.get_private_key(plan.from)
          ...
          payload = %{
            to: plan.to,
            gas_limit: plan.gas_limit |> to_hex,
            gas_price: (total_gas * 1_000_000_000) |> to_hex,
            from: plan.from,
            value: 0,
            data: "0x" <> data
          }
      ...
      {:error, _} ->
        update_transaction_plan(plan, %{
          "status" => "postponed",
          "remark" => "Cannot get base price. Network issue. "
        })
    end
  end
  ...
  def reset_undefined_plan(transaction_batch_id) do
    ...
    Repo.all(query)
    |> Enum.map(fn x ->
      update_transaction_plan(x, %{
        "hash" => nil,
        "status" => "unstarted",
        "attempt" => (x.attempt || 0) + 1
      })
    end)
  end
  ...
  def release_pending_whitelist_tx(transaction_plan_id, nonce, gas_price) do
    ...
    payload = %{
      ...
      nonce: nonce,
      gas_limit: plan.gas_limit |> to_hex,
      gas_price: (gas_price * 1_000_000_000) |> to_hex,
      from: plan.from,
      value: 0,
      data: "0x" <> data
    }
  ...
  def test(id) do
    ...
    create_send_eth_plan(
      "0xF86613BCF16C855446409F7F40A1AD9D9AB70A49",
      address.eth_address,
      0.0015,
      3,
      batch.title,
